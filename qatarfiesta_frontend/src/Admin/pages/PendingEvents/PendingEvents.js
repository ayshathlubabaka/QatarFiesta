...
function PendingEvents() {
    ...
    const handleCreditWallet = async(event_id) => {
      try {
        const response = await fetch(`${baseURL}/api/v1/admin/credit-wallet/${event_id}/`, {
          method: 'POST',
          headers: {
      ...
      <nav className="navbar navbar-light" style={{margin:"50px"}}>
        ...
        <div className="dropdown" style={{ marginRight: "350px" }}>
          ...
          <ul className="dropdown-menu" aria-labelledby="dropdownMenuButton1">
            ...
            <button className="dropdown-item" onClick={() => handleFilter('rejected')}>
              Not Approved
            </button>
            <button className="dropdown-item" onClick={() => handleFilter('approved')}>
              Approved
            </button>
          </ul>
        </div>
          ...
          <tbody>
            {sortedData.map((item) => (
              <tr key={item.id}>
                ...
                <td onClick={() => handleViewEvent(item.id)} style={{ fontWeight: "bolder", cursor: "pointer" }}>
                  {item.title}
                </td>
                <td>{item.venue}</td>

    ...
    <div>
      <div className={state===false ? 'navbar2' : 'navbar1'} >
        <div className="logo-container" onClick={()=>navigate('/')} style={{cursor:"pointer"}}>
          <img className="logo" src='../../../Image-1 (4).jpg' alt="QatarFiesta Logo" style={{height:"50px", width:"170px"}}/>
        </div>
        ...
        <span className='menu' onClick={()=>setState(!state)}><GiHamburgerMenu />{console.log(state)}</span>
          <div className={state===false ? 'navbar2-items' : "navbar1-items" }>
            <ul style={{cursor:"pointer"}}>
              ...
              <li>
  <Form inline onSubmit={handleSubmitSearch} className="d-flex align-items-center">
    <FormControl type="text" placeholder="Search" className="mr-sm-2" value={searchTerm}
          onChange={handleSearchChange} />
    <Button variant="outline-light" className="ml-2" type="submit"><FaSearch /></Button>
  </Form>
</li>
              ...
              <div>
                <Dropdown>
                  ...
                  <Dropdown.Menu>
                    ...
                    <Dropdown.Item href="/my-booking/">Bookings</Dropdown.Item>
                    <Dropdown.Item href="/wallet/">Wallet</Dropdown.Item>
                    <Dropdown.Item><button className='logout_btn' onClick={handleLogout}>Logout</button></Dropdown.Item>
                  </Dropdown.Menu>
                </Dropdown>
            </div>
            ) : (

...
function Events() {
  ...
  const fetchEventData = async () => {
    try {
      const response = await fetch(
        `${baseURL}/api/v1/organizer/active-event/`,
        {
          method: "GET",
        }
      );
  ...
  return (
    <div
      className="row"
      style={{
        borderLeft: "solid 30px black",
        borderRight: "solid 30px black",
      }}
    >
      ...
      <div>
          ...
          {displayedItems.map((item, id) => (
            <div key={id} className="card mb-3 mx-3 p-2">
              ...
              <div className="card-body">
                ...
                <p>
                  <FaClock className="mr-2" /> {item.startTime} to{" "}
                  {item.endTime}
                </p>
                <hr className="black-line" />
                <button
        ...
        <div className="pagination d-flex justify-content-center mt-3 mb-2">
          {[...Array(Math.ceil(eventData.length / itemsPerPage)).keys()].map(
            (page) => (
              <button
                key={page}
                className={`btn ${

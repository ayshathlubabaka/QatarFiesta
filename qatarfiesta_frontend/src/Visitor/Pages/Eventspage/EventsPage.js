        ...
        <div >
          ...
          <ul className="list-group">
            {categoryData.map((item, id) => (
                <li className="list-group-item list-group-item-action d-flex justify-content-between align-items-center category list1" style={{background:"#D3D3D3"}} onClick={() => handleFilter(item.id)}>
                {item.name}<span className="badge badge-primary badge-pill"></span>
              </li>
            ))}
            <li className="list-group-item list-group-item-action d-flex justify-content-between align-items-center category list1" style={{background:"#D3D3D3"}} onClick={()=>handleFilter('all')}>All<span className="badge badge-primary badge-pill"></span></li>
        ...
        <div className='d-flex flex-wrap justify-content-center mt-2'>
          {displayedItems.map((item, id) => (
            <div key={id} className="card mb-3 mx-3" style={{ width: '30rem' }}>
              <img className="card-img-top" src={item.image} alt="Card image cap" style={{ height: "15rem" }} />
  ...
  return (
      ...
      <div className="row justify-content-center" style={{ height: "100px" }}>
        {categoryData.map((item, id) => (
          <div key={id} className="col-md-1">
            <ul className="list-group">
              <li className="list-group-item list-group-item-action justify-content-between category" style={{ background: randomColor(), fontSize: "12px", fontWeight: "bold" }} >
                {item.name}
              </li>
            </ul>
          </div>
    ...
    <div className="row">
      ...
      <div className="col-md-2 ml-5">
        ...
        <button className="btn btn-dark mt-5" onClick={clearFilters} >Clear All Filters</button>
    
      </div>
      <div className='col-md-9 border border-dark mb-2'>  
      <EventListing eventData={eventData}/>
      <div className="pagination d-flex justify-content-center mt-3">
        {[...Array(Math.ceil(filteredEventData.length / itemsPerPage)).keys()].map((page) => (
          <button
            key={page}

...
function Wallet() {
    ...
    const baseURL = process.env.REACT_APP_API_BASE_URL
      ...
      const fetchTransactionData = async() => {
        try {
          const response = await fetch('http://127.0.0.1:8000/api/stripe/transaction/', {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${localStorage.getItem('access_token')}`,
          },
  ...
  return (
    ...
    <div className='row' style={{margin:"100px"}}>
        ...
        {walletData ? (
          <div className='row' style={{background:"#ffd7b5", color:"#ff6700"}}>
        ...
        <table className="table">
          <thead>
            <tr className='table-heading'>
              <th style={{ background: '#2b495f', color: 'white' }}>Date</th>
              <th style={{ background: '#2b495f', color: 'white' }}>Type</th>
              <th style={{ background: '#2b495f', color: 'white' }}>Amount</th>

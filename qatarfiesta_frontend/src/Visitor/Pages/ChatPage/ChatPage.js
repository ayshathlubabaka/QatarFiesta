...
function ChatPage() {
  ...
  const fetchEventData = async () => {
    try {
      ...
      const selectedObject = result.find((event) => event.id === parseInt(eventId));
  ...
  return (
    ...
    <div className='container-fluid, border border-dark' style={{ maxWidth: '60%', margin:"100px", marginLeft:"300px" }} >
      <button className='btn btn-danger mt-2'  onClick={handleCloseChat}><FaTimes /></button>
         <ListItemText primary={eventData.title}>{eventData.title}</ListItemText>
         
      <Divider />
      <List className={classes.messageArea} sx={{overflowY:'scroll',height:'50vh'}}>
          ...
          {userChatHistory.map((message, index) => (
              ...
              <Grid container>
                  ...
                  <Grid item xs={12}>
                  <ListItemText align={!message.from_user ? 'right' : 'left'} secondary={message.timestamp}></ListItemText>
                  </Grid>
              </Grid>
              </ListItem>
          ))}
...
<Grid item xs={1} align="right">
  ...
  >
    ...
    <Box justifyContent={'center'}>
      ...
      <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR51aVXwnmZhzP_PZzW9IVQRMok5e5z4tUBQHfVPS0--Z_ADwV7NI2zB_f-QkPrpBKAJpE&usqp=CAU" alt="" style={{maxWidth:'75%'}}/>

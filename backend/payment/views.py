...
from .serializer import EventBookingSerializer, walletViewSerializer, MyTransactionSerializer
from organizer.models import Events
...
class StripeCheckoutView(APIView):
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse(
            {"session_id": checkout_session.id, "stripe_public_key": settings.STRIPE_PUBLIC_KEY, "session_url":checkout_session.url}
        )
...
class CancelBookingAPI(APIView):
    def put(self, request, booking_id):
        try:
            ...
            return Response({'error': 'Something went wrong when cancelling booking'},
                                status=status.HTTP_500_INTERNAL_SERVER_ERROR)

...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        await self.save_message(handle_user_id=current_user_id,message=message,from_user=from_user,group_name=self.room_group_name)
    ...
    def save_message(self, handle_user_id, message, from_user, group_name):
        VisitorOrganizerChat.objects.create(handle_user_id=handle_user_id, message=message, group_name=group_name, from_user=from_user)
    ...
    def send_mail(self, current_user_id, event_id, organizer_id):
        websocket_connected.send(sender=self.__class__, user_id=current_user_id, event_id=event_id, organizer_id=organizer_id)
    ...
    def create_pending_chat(self, current_user_id, event_id, organizer_id):
        ...
        PendingChat.objects.create(reciever=organizer, chat_url=chat_url, user=user, event=event)
